...
export const SignUp = async (req, res) => {
     ...
     const userExists = await User.findOne({
          //  [{username}, {email}] 
          $or: [{email}] // replace on Firebase OAuth method : generate username if needed or use default specs provided by firebase ids, displayname etc
     });
     // if(userExists) {
     ...
     // }
      if(userExists) {
          return res.status(StatusCodes.BAD_REQUEST).json({message: "Such email already exists"}); // should be improved with custom error class or additional library for error handling 
     }
